...
**업데이트 확인**으로 앱과 지침의 최신 승인 버전을 함께 확인합니다. 기존 앱에서 1.2.1로 업데이트할 때는 **앱 설치 및 재시작…**을 먼저 완료한 뒤, 다시 열린 앱에서 **설치 미리보기 → 적용… → 확인**으로 새 지침을 적용합니다. 앱 업데이트와 지침 적용은 별도 단계입니다. 서명과 파일 해시를 검증한 뒤 설치하며 이전 앱과 설정은 백업합니다.
...
- Claude 하단 모델·추론·컨텍스트 표시와 공통 환경 설정. 모델(`model`)과 추론 강도(`effortLevel`, `modelSettings`)는 Claude Code의 `/model`·`/effort`가 저장하는 Mac별 설정이라 기존 값을 보존하고, 값이 없을 때만 기본값(Opus, Opus 5.5는 medium)을 넣습니다
...
`computer-use`, `orca-cli`, `orchestration`은 전역 경로에 새로 설치하지 않습니다. 기존 항목은 Orca 내부에서도 같은 경로를 사용하므로 변경·삭제하지 않으며, 밖에서 계속 보일 수 있습니다. 이번 버전은 완전한 외부 숨김이나 격리를 제공하지 않습니다. 보존된 Orca 정책 파일도 공통 지침에서 자동으로 읽도록 연결하지 않습니다.
