...
**업데이트 확인**으로 앱과 지침의 최신 승인 버전을 함께 확인합니다. 기존 앱에서 1.4.0으로 업데이트할 때는 **앱 설치 및 재시작…**을 먼저 완료한 뒤, 다시 열린 앱에서 **설치 미리보기 → 적용… → 확인**으로 새 지침을 적용합니다. 앱 업데이트와 지침 적용은 별도 단계입니다. 서명과 파일 해시를 검증한 뒤 설치하며 이전 앱과 설정은 백업합니다.
...
Codex `$orch`와 Claude `/orch`도 명시적으로 구성 선택을 요청하는 경로입니다. 대화에서는 번호가 모두 담긴 화면이 한 번 나오고, 한 줄로 답하면 바로 저장합니다. 예: `2`(절약형 그대로), `2 4-1-3`(어려운 구현을 GPT Astra·high로). `$orch 2 4-1-3`처럼 호출할 때 붙여도 됩니다. DeepSeek 일반 워커의 기본 추론은 max입니다. `$bundam`/`/bundam`은 현재 호스트에서 실제 작업을 나누는 별도 진입점으로 유지합니다. 스킬 이름의 단순 언급·문서 검토는 실행 요청이 아닙니다.
