...
**업데이트 확인**으로 앱과 지침의 최신 승인 버전을 함께 확인합니다. 기존 앱에서 1.4.2로 업데이트할 때는 **앱 설치 및 재시작…**을 먼저 완료한 뒤, 다시 열린 앱에서 **설치 미리보기 → 적용… → 확인**으로 새 지침을 적용합니다. 앱 업데이트와 지침 적용은 별도 단계입니다. 서명과 파일 해시를 검증한 뒤 설치하며 이전 앱과 설정은 백업합니다.
...
지금 실행 중인 프로그램이 지원하는 기능을 먼저 씁니다. `computer-use`, `orca-cli`, `orchestration`은 Orca 소유라 파일은 그대로 두지만, Orca 밖(herdr·일반 터미널)에서는 Claude 훅이 자동 호출을 막고 Codex 실행 명령이 목록에서 뺍니다. 필요하면 `/computer-use`처럼 직접 부를 수 있습니다. `ego-browser`는 직접 부를 때만 씁니다(Claude `/ego-browser`, Codex `$ego`). ego 앱이 업데이트하면서 Codex 쪽 스킬 연결을 다시 만들어도 Codex 실행 명령이 자동 사용 목록에서 뺍니다. herdr 연동은 herdr가 관리합니다(`herdr integration install claude`). 보존된 Orca 정책 파일도 공통 지침에서 자동으로 읽도록 연결하지 않습니다.
...
Codex `$orch`와 Claude `/orch`도 명시적으로 구성 선택을 요청하는 경로입니다. 번호 답 없이 부르면 브라우저에 선택 페이지가 열립니다. 드롭다운·버튼·＋/－로 고른 뒤 **코드 복사**한 한 줄을 대화에 붙여 넣으면 바로 저장합니다(페이지는 저장하지 않습니다). 브라우저를 열 수 없으면 번호가 모두 담긴 글 화면이 나오며 코드 형식은 같습니다. 예: `2`(절약형 그대로), `2 4-1-3`(어려운 구현을 GPT Astra·high로). `$orch 2 4-1-3`처럼 호출할 때 붙여도 됩니다. DeepSeek 일반 워커의 기본 추론은 max입니다. `$bundam`/`/bundam`은 현재 호스트에서 실제 작업을 나누는 별도 진입점으로 유지합니다. 스킬 이름의 단순 언급·문서 검토는 실행 요청이 아닙니다.
